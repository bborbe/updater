...
# --- Unified updater entry point ---


async def _run_go_modules(
    modules_args: list[str],
    project_type: str = "go",
    update_deps: bool = True,
) -> int:
    """Discover Go modules and process each one."""
    print("=== Discover Go Modules ===\n")

    module_paths = []
    for path_str in modules_args:
        path = Path(path_str).resolve()
        if not path.exists():
            print(f"✗ Path does not exist: {path}")
            return 1
    ...
    for i, mod in enumerate(modules, 1):
        if len(modules) > 1:
            print(f"\n[{i}/{len(modules)}] {mod.name}")
        success, _ = await process_module_with_retry(
            mod, project_type=project_type, update_deps=update_deps
        )
        if not success:
            return 1
...
async def _run_python_modules(modules_args: list[str]) -> int:
    """Discover Python modules and process each one."""
    print("=== Discover Python Modules ===\n")
...
async def _run_docker_modules(modules_args: list[str]) -> int:
    """Discover Docker projects and update base images."""
    print("=== Docker Image Updates ===\n")
...
async def _run_all_modules(modules_args: list[str]) -> int:
    """Discover all module types and process each one (same logic as main_async)."""
    # Setup early logging before auth so failures are diagnosable
    setup_module_logging(Path.cwd())
    ...
    if total_modules == 1:
        module, project_type = all_modules[0]
        lang = (
            "Go" if project_type == "go" else ("Python" if project_type == "python" else "Docker")
        )
        print(f"=== Updating {lang} Module: {module} ===\n")
        success, status = await process_module_with_retry(module, project_type=project_type)
        play_completion_sound()
...
async def main_updater_async() -> int:
    """Unified updater entry point with subcommands.
    ...
    for name, desc in _sub_descs.items():
    ...
    if args.subcommand == "go":
        ...
        return await _run_all_modules(args.modules)
    elif args.subcommand == "go-only":
        return await _run_go_modules(args.modules, project_type="go", update_deps=False)
    elif args.subcommand == "go-with-deps":
        return await _run_go_modules(args.modules, project_type="go", update_deps=True)
    elif args.subcommand == "python":
        return await _run_python_modules(args.modules)
    elif args.subcommand == "docker":
        return await _run_docker_modules(args.modules)
