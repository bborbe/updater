---
status: completed
container: updater-034-docker-full-pipeline
dark-factory-version: v0.107.5
created: "2026-04-08T06:40:41Z"
queued: "2026-04-08T06:46:51Z"
started: "2026-04-08T06:46:52Z"
completed: "2026-04-08T06:50:43Z"
---
...
2. In `PrecommitStep.run` (`src/updater/pipeline.py` around line 273), add an explicit `project_type == "docker"` branch. Final branch order must be: `CHECK_COMMAND → python → docker → go (fallthrough)`. For the docker branch, run `make precommit` via `run_command("make precommit", cwd=module_path, quiet=True, log_func=log_message)`. Do not touch the existing `CHECK_COMMAND` or `python` branches.
3. In `src/updater/cli.py` function `process_module_with_retry` (around line 341), replace the entire `elif project_type == "docker":` block that currently constructs a 2-step `Pipeline([DockerUpdateStep(), DockerCommitStep()])` with a single call: `success, status = await process_single_docker_module(module_path)`. Remove the inline `from .pipeline import DockerCommitStep, DockerUpdateStep, Pipeline, StepStatus` only if those names are not referenced elsewhere in that function body — grep within the function before deleting.
...
5. The old `DockerCommitStep` class in `src/updater/pipeline.py` (around line 640) is now unused. Before removing it, grep the entire repo: `grep -rn "DockerCommitStep" src/ tests/`. If nothing else references it, delete the class and its imports. If `tests/` or other code references it, leave it in place and add a `# TODO: remove after full-pipeline migration` comment above the class.
6. Add a pytest test in `tests/test_docker_pipeline.py`. `process_single_docker_module` is async, so mark tests with `@pytest.mark.asyncio` (or use the existing async test helper pattern — check `tests/` for how other async pipeline tests are written). Mock `verify_claude_auth`, `analyze_changes_with_claude`, and `play_completion_sound`. The test must:
